-- ============================================================================
-- clinicai-v2 · FIX Secretaria "Aguardando" · effective-reply
-- migration 199 · base canônica vigente = mig 147 (= definição em produção;
--   nenhuma migration entre 148 e 198 toca esta view).
-- ----------------------------------------------------------------------------
-- BUG: wa_conversations_operational_view.is_aguardando exigia
--      `ai_enabled = false` E `unread_count > 0`.
--      ai_enabled e unread_count NÃO são prova de resposta:
--        - ai_enabled = true  → "Lara PODE responder", não "Lara respondeu".
--        - unread_count = 0   → estado de leitura/sync, não resposta operacional.
--      Resultado: conversas onde a Lara está ligada mas NUNCA respondeu
--      (last_lara_msg IS NULL) ficavam ocultas → "Aguardando" mostrava 0
--      mesmo com pacientes esperando há minutos/dias (ex.: Patricia ~99min).
--
-- REGRA CORRETA (enterprise):
--      Aguardando = paciente falou por último E ninguém respondeu depois,
--      onde "resposta efetiva" = max(humano, Lara).
--        patient_last_at         = COALESCE(last_inbound_msg, last_inbound_time, last_lead_msg)
--        last_effective_reply_at = GREATEST(COALESCE(last_human_msg,epoch), COALESCE(last_lara_msg,epoch))
--        is_waiting              = status='active'
--                                   AND patient_last_at IS NOT NULL
--                                   AND patient_last_at > last_effective_reply_at
--      - Lara respondeu depois   → NÃO aguardando.
--      - Humano respondeu depois  → NÃO aguardando.
--      - Lara ligada mas não respondeu → AGUARDANDO.
--      - Não depende de unread_count nem de ai_enabled.
--
-- ESCOPO: muda SÓ is_aguardando, is_urgente, response_color (mesma base).
--   Preserva colunas/ordem/grants/security_invoker, filtros anti-internal,
--   archived/cross_loop e owner normalization (mig 145/146/147).
--   Não toca sla.ts/computeSla. Não faz UPDATE/DELETE/backfill/trigger/RPC.
-- ============================================================================
...
WITH msg_rollup AS (
  ...
  SELECT c.id,
    ...
    -- ── EFFECTIVE-REPLY (fix) · single source pra is_aguardando/is_urgente/response_color ──
...
SELECT id,
  ...
  false AS is_mira,
  -- ── FIX · is_aguardando = effective-reply (sem ai_enabled, sem unread_count) ──
  is_waiting_effective_reply AS is_aguardando,
  -- ── FIX · is_urgente = mesma base + threshold de tempo ──
  is_waiting_effective_reply AND (now() - patient_last_at) > '00:05:00'::interval AS is_urgente,
  -- ── FIX · response_color = mesma base ──
  CASE
    WHEN NOT is_waiting_effective_reply THEN 'none'::text
