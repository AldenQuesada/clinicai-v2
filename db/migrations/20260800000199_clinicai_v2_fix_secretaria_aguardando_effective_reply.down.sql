-- ============================================================================
-- DOWN · migration 199 · reverte o fix effective-reply.
-- Restaura wa_conversations_operational_view à definição vigente pré-199
-- (= mig 147 · is_aguardando com gates ai_enabled=false + unread_count>0).
-- ============================================================================
...
WITH (security_invoker = true) AS
 WITH msg_rollup AS (
        ...
        UNION ALL
         SELECT wn.clinic_id,
            (SUBSTRING(norm.d FROM 1 FOR 4) || '9'::text) || SUBSTRING(norm.d FROM 5) AS phone_key
           FROM wa_numbers wn
             CROSS JOIN LATERAL ( SELECT regexp_replace(COALESCE(wn.phone, ''::text), '\D'::text, ''::text, 'g'::text) AS d) norm
          WHERE length(norm.d) = 12
        UNION ALL
         SELECT wn.clinic_id,
            "right"(norm.d, 8) AS phone_key
           FROM wa_numbers wn
             CROSS JOIN LATERAL ( SELECT regexp_replace(COALESCE(wn.phone, ''::text), '\D'::text, ''::text, 'g'::text) AS d) norm
          WHERE length(norm.d) >= 8
        ), base AS (
         SELECT c.id,
          ...
          WHERE c.deleted_at IS NULL AND w.id = 'ead8a6f9-6e0e-4a89-8268-155392794f69'::uuid AND w.inbox_role = 'secretaria'::text AND w.is_active IS TRUE AND COALESCE(c.metadata ->> 'archived_at'::text, ''::text) = ''::text AND COALESCE(c.metadata ->> 'archived_reason'::text, ''::text) <> 'cross_internal_loop'::text AND NOT (EXISTS ( SELECT 1
                   FROM internal_phone_keys ipk
                  WHERE ipk.clinic_id = c.clinic_id AND (ipk.phone_key = regexp_replace(COALESCE(c.phone, ''::text), '\D'::text, ''::text, 'g'::text) OR ipk.phone_key = "right"(regexp_replace(COALESCE(c.phone, ''::text), '\D'::text, ''::text, 'g'::text), 8))))
        )
 SELECT id,
    clinic_id,
    lead_id,
    ...
    false AS is_mira,
    status = 'active'::text AND COALESCE(ai_enabled, false) = false AND COALESCE(unread_count, 0) > 0 AND COALESCE(last_inbound_msg, last_inbound_time, last_lead_msg) IS NOT NULL AND COALESCE(last_inbound_msg, last_inbound_time, last_lead_msg) > COALESCE(last_human_msg, '1970-01-01 00:00:00+00'::timestamp with time zone) AS is_aguardando,
    status = 'active'::text AND COALESCE(ai_enabled, false) = false AND COALESCE(unread_count, 0) > 0 AND COALESCE(last_inbound_msg, last_inbound_time, last_lead_msg) IS NOT NULL AND COALESCE(last_inbound_msg, last_inbound_time, last_lead_msg) > COALESCE(last_human_msg, '1970-01-01 00:00:00+00'::timestamp with time zone) AND (now() - COALESCE(last_inbound_msg, last_inbound_time, last_lead_msg)) > '00:05:00'::interval AS is_urgente,
        CASE
            WHEN NOT (status = 'active'::text AND COALESCE(ai_enabled, false) = false AND COALESCE(unread_count, 0) > 0 AND COALESCE(last_inbound_msg, last_inbound_time, last_lead_msg) IS NOT NULL AND COALESCE(last_inbound_msg, last_inbound_time, last_lead_msg) > COALESCE(last_human_msg, '1970-01-01 00:00:00+00'::timestamp with time zone)) THEN 'none'::text
            WHEN (now() - COALESCE(last_inbound_msg, last_inbound_time, last_lead_msg)) > '00:30:00'::interval THEN 'critico'::text
            WHEN (now() - COALESCE(last_inbound_msg, last_inbound_time, last_lead_msg)) > '00:05:00'::interval THEN 'vermelho'::text
