      ...
      <form className="rounded-lg border border-[#C9A96E]/22 bg-[#C9A96E]/[0.04] px-3.5 py-3 flex items-center gap-2.5 flex-wrap">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <input
            ...
            defaultValue={phone}
            placeholder="ex: 5511..."
            className="px-2.5 py-1.5 rounded-lg bg-white/[0.02] border border-white/10 text-xs text-[#F5F0E8] font-mono focus:outline-none focus:border-[#C9A96E]/50 w-36"
          />
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <input
            ...
            className="px-2.5 py-1.5 rounded-lg bg-white/[0.02] border border-white/10 text-xs text-[#F5F0E8] font-mono focus:outline-none focus:border-[#C9A96E]/50 w-44"
          />
        </div>
      ...
      {logs.length === 0 ? (
        ...
        <div className="flex flex-col gap-0.5 max-h-[600px] overflow-y-auto custom-scrollbar">
          {logs.map((l) => (
            <div
              key={l.id}
              className="grid grid-cols-[110px_140px_140px_1fr_auto_auto] gap-2.5 items-center px-3 py-2 rounded border border-transparent hover:bg-white/[0.02] hover:border-white/10 transition-colors text-[11px]"
            >
              <span className="text-[10px] font-mono text-[#9CA3AF] whitespace-nowrap">
                {fmtDateTime(l.createdAt)}
              </span>
              <span className="font-mono text-[11px] text-[#F5F0E8] truncate" title={l.phone}>
                {l.phone}
              </span>
              <span className="font-mono text-[11px] text-[#C9A96E] truncate" title={l.intent ?? ''}>
                {l.intent ?? '—'}
              </span>
              <span className="text-[11px] text-[#F5F0E8] truncate" title={l.query}>
                {l.query}
                {l.rpcCalled && (
        ...
        <div className="flex gap-1.5">
          {page > 1 && (
            <Link
              ...
              className="px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-[1px] border border-white/10 text-[#9CA3AF] hover:border-[#C9A96E]/40 hover:text-[#C9A96E] transition-colors"
            >
              ← Anterior
            </Link>
          )}
