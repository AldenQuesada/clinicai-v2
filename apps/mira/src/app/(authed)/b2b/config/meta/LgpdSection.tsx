/**
 * LgpdSection · Server Component extraido de /estudio/lgpd/page.tsx.
 *
 * Renderiza a UI de compliance LGPD (sugestoes de anonimizacao + ja-anon)
 * fetched dentro do proprio component. Usado pelo /b2b/config/meta/page.tsx
 * (fusao 2-col com SobreSection).
 *
 * Acoes destrutivas (anonymizePartnershipAction) seguem em
 * apps/mira/src/app/(authed)/estudio/lgpd/actions.ts · zero duplicacao.
 */

import { ShieldAlert, ShieldCheck } from 'lucide-react'
import { loadMiraServerContext } from '@/lib/server-context'
import { anonymizePartnershipAction } from '../../../estudio/lgpd/actions'
...
export async function LgpdSection() {
  ...
  return (
    <div className="flex flex-col gap-3.5">
      {/* Aviso */}
      <div className="rounded-lg border border-[#FCA5A5]/30 bg-[#FCA5A5]/[0.04] p-3 flex gap-2.5 items-start">
        <ShieldAlert className="w-4 h-4 text-[#FCA5A5] shrink-0 mt-0.5" />
        <div className="flex-1">
          <div className="text-[11.5px] font-semibold text-[#FCA5A5]">Atenção · ação irreversível</div>
          <p className="text-[10.5px] text-[#9CA3AF] mt-1 leading-relaxed">
            A anonimização <strong>não pode ser desfeita</strong>. Nome, contato (telefone,
            email, instagram, site) e narrative_quote viram placeholders. Métricas agregadas
            (vouchers, attributions, comments) preservadas. Motivo informado vai pro audit_log.
          </p>
        </div>
      </div>
      ...
      <section className="flex flex-col gap-2">
        ...
        {candidates.length === 0 ? (
          <div className="rounded-lg border border-white/10 bg-white/[0.02] p-4 text-center text-[11px] text-[#9CA3AF]">
            Nenhuma parceria encerrada elegível · só status=closed aparece aqui.
          </div>
        ) : (
          candidates.map((p) => (
            <details
            ...
            >
              <summary className="cursor-pointer px-3 py-2 flex items-center justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <span className="text-[12px] text-[#F5F0E8] truncate">{p.name}</span>
                  {p.pillar && (
              ...
              <form
                ...
                className="px-3 pb-3 pt-2.5 flex flex-col gap-2.5 border-t border-[#FCA5A5]/15 bg-[#FCA5A5]/[0.04] rounded-b-lg"
              >
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full px-2.5 py-1.5 rounded-md bg-white/[0.02] border border-white/10 text-[11.5px] text-[#F5F0E8] resize-y focus:outline-none focus:border-[#FCA5A5]/50"
                  />
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.02] divide-y divide-white/10">
            {alreadyAnon.map((p) => (
              <div key={p.id} className="px-3 py-2 flex items-center justify-between gap-3">
                <span className="text-[11px] text-[#9CA3AF] font-mono">{p.name}</span>
