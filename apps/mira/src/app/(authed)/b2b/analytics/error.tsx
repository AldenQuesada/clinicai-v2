...
/**
 * Error boundary do segmento /b2b/analytics · captura crashes do Server
 * Component e expoe digest visivel pra debug em prod (NextJS 16 omite
 * .message em build production).
 *
 * Quando estavel · DELETAR ou substituir por uma mensagem premium estatica.
 */
  ...
  return (
    <main className="flex-1 overflow-y-auto bg-[hsl(var(--chat-bg))] p-6">
      <div className="max-w-[760px] mx-auto rounded-lg border border-[#EF4444]/40 bg-[#EF4444]/8 p-5 flex flex-col gap-3">
        <h2 className="text-[#FCA5A5] text-lg font-bold">
          ⚠ Erro ao carregar Analytics
        </h2>
        <p className="text-[#F5F0E8] text-sm">
          O Server Component crashou. Em prod o NextJS omite a mensagem original ·
          procure este digest nos Runtime Logs do Vercel pra ver a stack trace
          completa.
        </p>
        <div className="rounded border border-white/10 bg-black/30 px-3 py-2 font-mono text-xs">
          ...
          {error.stack && process.env.NODE_ENV !== 'production' && (
            <>
              <div className="text-[#9CA3AF] mt-2">stack:</div>
              <pre className="text-[#9CA3AF] text-[10px] whitespace-pre-wrap">
                {error.stack}
              </pre>
            </>
          )}
