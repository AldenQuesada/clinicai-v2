  ...
  return (
    <div className="w-80 border-r border-[hsl(var(--chat-border))] flex flex-col bg-[hsl(var(--chat-panel-bg))] relative">
      <div className="h-[72px] border-b border-[hsl(var(--chat-border))] flex items-center px-4 justify-between shrink-0">
        <span className="text-sm font-medium">Conversas ({filteredConversations.length})</span>
        <div className="flex items-center gap-2">
           <button 
            onClick={() => setSortOrder(prev => prev === 'newest' ? 'oldest' : 'newest')}
            className={`p-1.5 rounded-md transition-colors ${sortOrder === 'oldest' ? 'text-blue-500 bg-blue-500/10' : 'text-gray-400 hover:text-white'}`}
            title="Inverter Ordem"
          >
            <ArrowUpDown className="h-4 w-4" />
          </button>
          <MessageSquarePlus className="h-5 w-5 text-gray-400 cursor-pointer hover:text-white transition-colors" />
        </div>
      </div>
      ...
      <div className="p-3 border-b border-[hsl(var(--chat-border))] shrink-0 flex flex-col gap-3">
            ...
            <button
              ...
              className={`px-2 py-1 text-[9px] font-bold uppercase tracking-wider rounded-md transition-all whitespace-nowrap ${
                statusFilter === s.id 
                  ...
                  : 'bg-[hsl(var(--chat-bg))] text-gray-500 hover:text-gray-300'
              }`}
            >
              {s.label}
            </button>
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-[hsl(var(--chat-bg))] border border-[hsl(var(--chat-border))] rounded-md py-2 pl-9 pr-4 text-sm focus:outline-none focus:ring-1 focus:ring-[hsl(var(--ring))]"
            />
          </div>
          <button 
            ...
            className={`p-2 rounded-md border transition-all ${
              showFilters || hasActiveAdvancedFilters
                ...
                : 'bg-[hsl(var(--chat-bg))] border-[hsl(var(--chat-border))] text-gray-400 hover:text-white'
            }`}
          >
        ...
        {showFilters && (
          <div className="bg-[hsl(var(--chat-panel-bg))] border border-[hsl(var(--chat-border))] rounded-lg p-3 space-y-3 shadow-2xl z-20 animate-in fade-in slide-in-from-top-2">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Filtros</span>
            ...
            <div className="space-y-1">
              <label className="text-[9px] text-gray-400 flex items-center gap-1 uppercase font-bold">
                <Target className="h-3 w-3" /> Funil
              </label>
              <select 
                value={filterFunnel}
                onChange={(e) => setFilterFunnel(e.target.value)}
                className="w-full bg-[hsl(var(--chat-bg))] border border-[hsl(var(--chat-border))] text-xs rounded px-2 py-1 outline-none text-gray-200"
              >
            ...
            <button 
              ...
              className="w-full py-1.5 text-[10px] text-gray-400 hover:text-red-400 hover:bg-red-400/5 transition-all border border-dashed border-gray-700 rounded font-medium"
            >
              Limpar Filtros
            </button>
          </div>
            ...
            {tabs.map(tab => (
              <button
                ...
                className={`flex-1 text-[11px] font-medium py-1.5 rounded-sm transition-colors cursor-pointer ${
                  activeTab === tab 
                    ? 'bg-[hsl(var(--chat-panel-bg))] text-white shadow-sm' 
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {isLoading ? (
          <div className="p-10 text-center flex flex-col items-center gap-3">
             ...
             <span className="text-xs text-gray-500">Carregando conversas...</span>
          </div>
          ...
          filteredConversations.map((conv) => (
            <div
              ...
              className={`p-4 border-b border-[hsl(var(--chat-border))] cursor-pointer transition-colors hover:bg-[hsl(var(--chat-bg))] ${
                selectedConversation?.conversation_id === conv.conversation_id ? 'bg-[hsl(var(--chat-bg))] border-l-2 border-l-blue-500' : ''
              }`}
            >
              <div className="flex items-start gap-3">
                <div className="relative shrink-0">
                  ...
                  {conv.channel === 'cloud' && (
                    <div className="absolute -bottom-0.5 -right-0.5 bg-green-500 rounded-full p-0.5 border-2 border-[hsl(var(--chat-panel-bg))] shadow-md" title="WhatsApp Cloud API">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 truncate mt-0.5">{conv.last_message_text || 'Sem mensagens'}</p>
                  
                  <div className="mt-2 flex flex-wrap gap-1">
                    {conv.phase && (
                      <div className="text-[10px] bg-blue-500/10 text-blue-500 px-2 py-0.5 rounded-full inline-block">
                        {conv.phase}
                      </div>
                    )}
                    {conv.is_urgent && (
                      <div className="text-[10px] bg-red-500/10 text-red-500 px-2 py-0.5 rounded-full inline-block">
                        Urgente
                      </div>
                    )}
                  </div>
                  ...
                  {conv.tags && conv.tags.length > 0 && (
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {conv.tags.slice(0, 3).map((tag, i) => (
                        <span key={i} className="text-[9px] bg-gray-500/10 text-gray-400 px-1.5 py-0.5 rounded border border-gray-500/20">
                          {tag}
                        </span>
                      ))}
                      {conv.tags.length > 3 && <span className="text-[9px] text-gray-500">+{conv.tags.length - 3}</span>}
                    </div>
                  )}
                </div>
              </div>
